      ...
      var recursy = function recursy(elem) {
        elem.childNodes.forEach(function (item) {
          if (item.nodeName === '#text' && item.nodeValue.replace(/\s+/g, "").length > 0) {
            if (!item.parentNode.getAttribute('data-fz')) {
              var value = window.getComputedStyle(item.parentNode, null).fontSize;
              item.parentNode.setAttribute('data-fz', +value.replace(/px/g, ""));
              item.parentNode.style.fontSize = item.parentNode.getAttribute('data-fz') * _this2.scale + 'px';
            } else {
              item.parentNode.style.fontSize = item.parentNode.getAttribute('data-fz') * _this2.scale + 'px';
            }
          } else {
...
window.addEventListener('DOMContentLoaded', function () {
  var customizator = new _customizator__WEBPACK_IMPORTED_MODULE_0__["default"]();
  customizator.render();
});
